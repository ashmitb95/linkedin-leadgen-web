/**
 * tanmay-job-run.ts — Job search pipeline for Tanmay.
 *
 * Same multi-source pipeline as job-run.ts but uses Tanmay's config/profile
 * and writes to tanmay_jobs/tanmay_job_runs tables.
 *
 * Usage:
 *   npx tsx scripts/tanmay-job-run.ts                    # Full run
 *   npx tsx scripts/tanmay-job-run.ts --quick            # Smoke test
 *   npx tsx scripts/tanmay-job-run.ts --max 2            # Limit searches
 *   npx tsx scripts/tanmay-job-run.ts --content-only     # Only LinkedIn hiring posts
 *   npx tsx scripts/tanmay-job-run.ts --jobs-only        # Only LinkedIn job listings
 *   npx tsx scripts/tanmay-job-run.ts --naukri-only      # Only Naukri.com
 *   npx tsx scripts/tanmay-job-run.ts --boards-only      # Only Naukri + Hirist
 *   npx tsx scripts/tanmay-job-run.ts --linkedin-only    # Only LinkedIn
 */
...
import { tanmayJobOps } from "../src/lib/pipeline/db-ops";
import { tanmayJobsDb } from "../src/lib/jobs";

const CONFIG_PATH = path.resolve(process.cwd(), "config", "tanmay-job-keywords.json");
const PROFILE_PATH = path.resolve(process.cwd(), "config", "tanmay-job-profile.json");
...
async function runSearch(
  ...
  try {
    ...
    console.log("    Scoring with Claude (Tanmay profile)...");
    ...
    for (const j of scoredJobs) {
      ...
      const { isNew } = await tanmayJobOps.upsertJob({
        dedup_key: dedupKey,
        source: job.mode,
        title: j.title,
        company: j.company,
...
async function main() {
  ...
  console.log(`\n=== Tanmay — Job Search Run [${modeLabel}] ===`);
  console.log(`Profile: ${profile.name || "Tanmay"} — Senior Frontend / Full Stack Engineer`);
  ...
  const runId = await tanmayJobOps.logJobRunStart();
  ...
  await tanmayJobOps.logJobRunEnd(runId, {
  ...
  const digest = await tanmayJobsDb.getJobDigest();
  console.log("\n" + digest);
}
