...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const job = await tanmayJobsDb.getJobById(id);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.notes !== undefined) {
    const updated = await tanmayJobsDb.updateJobNotes(id, body.notes);
  ...
  const updated = await tanmayJobsDb.updateJobStatus(id, body.status);
  if (!updated) return NextResponse.json({ error: "Job not found" }, { status: 404 });
  return NextResponse.json({ success: true });
}
